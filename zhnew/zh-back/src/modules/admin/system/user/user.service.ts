...
import { camelCase, includes, isEmpty } from 'lodash';
...
export class SysUserService {
  ...
  /**
   * 根据用户id或者名字 返回用户id
   * @param nameOrId 用户id/用户名
   */
  async findUserIdByNameOrId(nameOrId: string): Promise<SysUser> {
    const user = await this.userRepository.findOne({
      where: [
        { id: Number(nameOrId) ? Number(nameOrId) : null },
        { username: Like(`%${nameOrId}%`) },
        { name: Like(`%${nameOrId}%`) },
      ],
    });
    if (isEmpty(user)) {
  ...
  async info(
    ...
    if (isEmpty(departmentRow) && this.rootRoleId !== id) {
    ...
    return {
      ...
      roles,
      departmentName:
        departmentRow && departmentRow.name
          ? departmentRow.name
          : this.rootRoleId === id
          ? '超级管理员'
          : '未分配部门',
   ...
   *
   */
  async list(user): Promise<any> {
    let { lv, uid, departmentName } = user;
    const rootUserId = await this.findRootUserId();
    //查询 除了超管和自己的用户 并且 用户的部门名称和自己的部门名称一致
    //查询部门id
